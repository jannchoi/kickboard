...
def match_classes_with_colors(txt_folder, image_folder):
    ...
    for txt_file in os.listdir(txt_folder):
        if txt_file.endswith('.txt'):
            ...
            print(f"Most common object inside the blue box for {base_name}: {most_common_object}")
...
def add_grid_lines_and_bbox(image, class_2_coords, grid_size=32):
    ...
    print("class_2_coords:", class_2_coords)  # 확인을 위해 추가한 부분
...
def save_bbox_coordinates(txt_folder, image_folder, save_folder):
    for txt_file in os.listdir(txt_folder):
        if txt_file.endswith('.txt'):
            base_name = os.path.splitext(txt_file)[0]
            image_file = os.path.join(image_folder, base_name + '.jpg')
            ...
            for class_id, coords in class_coords.items():
                if class_id == 5:
                    class_5_coords.extend(coords)

            # 바운딩 박스 좌표 파일로 저장
            save_file = os.path.join(save_folder, base_name + '_bbox.txt')
            with open(save_file, 'w') as f:
                for coords in class_5_coords:
                    f.write(' '.join(map(str, coords)) + '\n')

# 폴더 경로
txt_folder = 'v8_5_results/labels'
image_folder = 'v8_5_results/images'
save_folder = 'v8_5_results/bounding5'  # 클래스 '5'의 바운딩 박스 좌표를 저장할 폴더

# 클래스 '5'의 바운딩 박스 좌표를 파일로 저장
save_bbox_coordinates(txt_folder, image_folder, save_folder)
