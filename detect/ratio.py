...
def match_classes_with_colors(txt_folder, image_folder):
    ...
    for txt_file in os.listdir(txt_folder):
        if txt_file.endswith('.txt'):
            ...
            if most_common_number == 5:  # sidewalk인 경우에만 텍스트 파일 저장
                if class_2_coords:
                    ...
                    save_coordinates_to_text(base_name, x_min, y_min_new, x_max, y_max_new)
...
def save_coordinates_to_text(base_name, x_min, y_min_new, x_max, y_max_new):  # 좌표를 텍스트 파일에 저장하는 함수
    output_file = f"{base_name}_coordinates.txt"
    with open(output_file, 'w') as file:
        file.write(f"{x_min},{y_min_new},{x_max},{y_max_new}\n")
    print(f"Coordinates saved to {output_file}")

    # 픽셀 좌표로 변환하여 저장
    image_width, image_height = x_max - x_min, y_max_new - y_min_new
    with open(f"{base_name}_pixel_coordinates.txt", 'w') as file:
        file.write(f"Pixel coordinates: ({x_min}, {y_min_new}) - ({x_max}, {y_max_new})\n")
